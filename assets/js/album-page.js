...
function findMostCommonColor(image) {
 ...
 for (let i = 0; i < data.length; i += 4) {
  // Ottieni i valori RGBA del pixel corrente
  const red = data[i];
  ...
  // Crea una stringa rappresentante il colore
  const color = `${red},${green},${blue}`;

  // Incrementa il conteggio per il colore corrente
 ...
 // Restituisci il colore più comune come array di valori RGB
 if (mostCommonColor) {
  return mostCommonColor.split(",").map(Number);
 } else {
  return null;
 }
}
...
image.onload = function () {
 const mostCommonColor = findMostCommonColor(image);
 console.log("Colore più presente:", mostCommonColor);
};
image.src = "path/dell/immagine.jpg";
